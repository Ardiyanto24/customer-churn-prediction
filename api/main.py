...
async def predict_single(input_data: CustomerInput):
    ...
    if not predictor._is_ready:
        return JSONResponse(
            ...
            content={
                "status": "error",
                "message": "Model belum siap menerima request. Coba lagi nanti.",
            },
        )
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={
                "status": "error",
                "message": "Terjadi kesalahan internal saat memproses prediksi.",
            },
        )
...
@app.post(
    ...
    tags=["Prediction"],
    summary="Predict churn for multiple customers (JSON)",
)
async def predict_batch_json(inputs: List[CustomerInput]):
    ...
    if len(inputs) == 0:
        return JSONResponse(
            status_code=422,
            content={"status": "error", "message": "List input tidak boleh kosong."},
        )
    ...
    if len(inputs) > 1000:
        return JSONResponse(
            ...
            content={
                "status": "error",
                "message": "Batas maksimum adalah 1000 pelanggan per request.",
            },
        )
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={
                "status": "error",
                "message": "Terjadi kesalahan internal saat memproses prediksi massal.",
            },
        )
...
@app.post(
    ...
    response_model=BatchPredictionResponse,
    tags=["Prediction"],
    summary="Predict churn from CSV file upload",
)
    ...
    try:
        ...
        if missing_cols:
            return JSONResponse(
                ...
                content={
                    "status": "error",
                    "message": f"Kolom wajib berikut hilang dari CSV: {missing_cols}",
                },
            )
        ...
        if not valid_inputs:
            return JSONResponse(
                ...
                content={
                    "status": "error",
                    "message": "Tidak ada baris data yang valid di dalam file CSV.",
                },
            )
        ...
        if len(valid_inputs) > 1000:
            return JSONResponse(
                ...
                content={
                    "status": "error",
                    "message": "Batas maksimum data yang valid adalah 1000 baris per file CSV.",
                },
            )
